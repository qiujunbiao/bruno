...
import { IconVariable, IconSettings, IconRun, IconFolder, IconDatabase, IconWorld, IconHome, IconFileCode, IconFileText } from '@tabler/icons';
  ...
  const getTabInfo = (type, tabName) => {
    switch (type) {
      ...
      case 'doc': {
        return (
          <>
            <IconFileText size={14} strokeWidth={1.5} className="special-tab-icon flex-shrink-0" />
            <span className="ml-1 tab-name">{tabName || 'Doc'}</span>
          </>
        );
      }
      case 'variables': {
